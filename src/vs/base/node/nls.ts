...
/**
 * Makes the bundled Simplified Chinese pack available on the very first run,
 * before the extension service has had a chance to create languagepacks.json.
 */
async function getBundledKunCodeLanguagePack(nlsMetadataPath: string): Promise<ILanguagePacks | undefined> {
	const extensionPath = join(nlsMetadataPath, '..', 'extensions', 'ms-ceintl.vscode-language-pack-zh-hans');
	try {
		const manifest = JSON.parse(await promises.readFile(join(extensionPath, 'package.json'), 'utf-8'));
		const localization = manifest?.contributes?.localizations?.find((item: { languageId?: string }) => item.languageId === 'zh-cn');
		...
		return {
			'zh-cn': {
				hash: `kuncode-bundled-${manifest.version}`,
