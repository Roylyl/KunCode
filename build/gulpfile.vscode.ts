...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const extensions = es.merge(
			...
			// KunCode's local extensions are source-controlled app customizations rather than marketplace packages.
			gulp.src([
		...
		if (platform === 'win32') {
			...
			const kuncodeIcon = gulp.src('resources/kuncode/app-icon.icns', { base: '.' });
			const policyDest = gulp.src('.build/policies/darwin/**', { base: '.build/policies/darwin' })
				.pipe(rename(f => f.dirname = `policies/${f.dirname}`));
			all = es.merge(all, shortcut, kuncodeIcon, policyDest);
		}
